...
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.08.07] - Black keypad driver feature is added.
#if defined(CONFIG_KEYBOARD_TWL4030) || defined(CONFIG_KEYBOARD_TWL4030_MODULE) || defined(CONFIG_KEYBOARD_TWL4030_BLACK)
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.08.07]- Black keypad driver feature is added.
...
/* 20110621, mschung@ubiquix.com, HBJ_GB One Source. [UB_START] */
#if defined(CONFIG_PRODUCT_LGE_HUB)||defined(CONFIG_PRODUCT_LGE_JUSTIN)
		if (i == 4)
			twl->address = 0x6a;
#endif
/* 20110621, mschung@ubiquix.com, HBJ_GB One Source. [UB_END] */
...
//--[[ LGE_UBIQUIX_MODIFIED_START : scchoi@ubiquix.com [2011.07.14] - Merged with hub-froyo's source.
// 20100816 taehwan.kim@lge.com Disable VBAT power on [START_LGE]
#if 1
#if defined(CONFIG_PRODUCT_LGE_HUB)
    {
// 20100817 sookyoung.kim@lge.com Change STARTON_CONDITION from 0xB3 to 0xF3 to make SMPL works.
// PM_MASTER_CFG_P1_TRANSITION = SWBUG, RES, VBUS, VBAT, RTC, USB, CHG, PWON.
// 0xF3 makes the system Wakeup or boot upon SWBUG, RES, VBUS, VBAT, CHG, PWON. I.e., VBAT is added.
// The three STARTON_CONDITIONs should be the same.
// bootable/bootloader/u-boot/drivers/twl4030.c
// bootable/bootloader/u-boot/board/hub/hub.c
// kernel/android-2.6.32/drivers/mfd/twl-core.c
#define     STARTON_CONDITION   0xF3    // Wakeup or boot upon SWBUG, RES, VBUS, VBAT, CHG, PWON.
        u8 val = 0;
        u8 count = 0;
        ...
        if(val != STARTON_CONDITION) /* except STARTON_VBAT (w/o SMPL), STARTON_RTC, STARTON_USB */
        {
            unprotect_pm_master();
            ...
            {
                val = 0;
                twl_i2c_read_u8(TWL4030_MODULE_PM_MASTER, &val, count);
                printk("[Start on condition] Before: CFG_P%d_TRANSITION - 0x%x\n", count+1, val);
 
                twl_i2c_write_u8(TWL4030_MODULE_PM_MASTER, STARTON_CONDITION, count);
                val = 0;
                twl_i2c_read_u8(TWL4030_MODULE_PM_MASTER, &val, count);
                printk("[Start on condition] After: CFG_P%d_TRANSITION - 0x%x\n", count+1, val);
            }
            protect_pm_master();
        }
    }
...
#endif
// 20100816 taehwan.kim@lge.com Disable VBAT power on [START_LGE]
//--]] LGE_UBIQUIX_MODIFIED_END : scchoi@ubiquix.com [2011.07.14] - Merged with hub-froyo's source.
