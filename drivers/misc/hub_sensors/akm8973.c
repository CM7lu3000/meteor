...
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.06.30] - Init polling delay modified.
static short akmd_delay = 200; //20101110 taehwan.kim@lge.com Default Slow Rate
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.06.30]- Init polling delay modified.
...
/* S, 20110813, mschung@ubiquix.com, Debug LCD distortion(blinking) at LCD resume. */
#if defined(CONFIG_PRODUCT_LGE_HUB)
static short temp_rwbuf;
short AKECS_temperature_value(void)
{
	return temp_rwbuf;
}
EXPORT_SYMBOL(AKECS_temperature_value);
#endif
/* E, 20110813, mschung@ubiquix.com, Debug LCD distortion(blinking) at LCD resume. */
...
//	printk("[shyun-kernel] report value [IN]\n");
	...
	/* Proximity */
	if(atomic_read(&p_flag)){
//		printk("[shyun] proximity event sending!\n");
		if(is_proxi_open()==1)
...
/* S, 20110813, mschung@ubiquix.com, Debug LCD distortion(blinking) at LCD resume. */
#if defined(CONFIG_PRODUCT_LGE_HUB)
	temp_rwbuf = rbuf[3]; //20101031 kyungtae.oh@lge.com for resume timing
#endif
/* E, 20110813, mschung@ubiquix.com, Debug LCD distortion(blinking) at LCD resume. */
	      ...
	      unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_APP_SET_DELAY:
		...
		#endif
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.07.01] - Sample rate feature added(From hub_froyo)
		if (akmd_delay != flag) {
            if (flag < 20) kxtf9_accel_api_set_sample_rate(3);
            else if (flag < 200) kxtf9_accel_api_set_sample_rate(2);
            else if (flag >= 200) kxtf9_accel_api_set_sample_rate(0);
            else kxtf9_accel_api_set_sample_rate(0);
        }
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.07.01]- Sample rate feature added(From hub_froyo)
		akmd_delay = flag;
		break;
...
//		printk("[shyun] ECS_IOCTL_SET_YPR [IN]\n");
		#if DEBUG
...
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.06.30] - MAGNETIC_VECTOR reporting error fixed.
	#if 1
	...
	#endif
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.06.30]- MAGNETIC_VECTOR reporting error fixed.
	/* proximity */
	...
	printk("[shyun] misc_register err = %d\n",err);
	if (err) {
