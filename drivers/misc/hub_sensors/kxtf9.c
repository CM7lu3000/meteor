...
#define DEBUG   0
#define ENHANCED_SNAP 0 //enhanced snap function, disabled taehwan.kim@lge.com
#define ENABLE_SHAKE 0 //To enable shake function, in Hub shake is disabled. 
                       //if shake is enabled, preventing function from cable insertion tapping is disabled.
                       //If you need both function, modification is required.
#define NO_COMPASS_SUPPORT 1 //support REV A without compass IC taehwan.kim@lge.com
...
//20101126 taehwan.kim@lge.com flip check during vibration suspend time when vibrating
     struct work_struct           flip_in_vibration; 
     struct work_struct           snap_work;
     struct work_struct           tap_defence_work;
     ...
     int vib_duration;
//20101126 taehwan.kim@lge.com flip check during vibration suspend time when vibrating
     struct workqueue_struct	 *flip_vib_wq;
     struct workqueue_struct     *snap_wq;
     struct workqueue_struct     *tap_defend_wq; //20101006 taehwan.kim@lge.com To defend tapping from Cable insertion 
...
{	
 ...
 void kxtf9_enable_irq(void)
 {
	 ...
	 if(data->use_irq == 0)
	 {
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.07.07] - Unnecessary log is removed.
//		 printk("[kxtf9_enable_irq] data->use_irq == 0\n");
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.07.07]- Unnecessary log is removed.
		 enable_irq(data->irq);
		 data->use_irq = 1;
	 }
 }
...
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.07.07]- Unnecessary log is removed
 ...
 void kxtf9_send_snap_detection(int value)
 {
 	struct kxtf9_data *data = i2c_get_clientdata(this_client);
#if defined(CONFIG_MACH_LGE_HUB_REV_A) //NO_COMPASS_SUPPORT
    if (system_rev == 1) report_rotation_direct(value);
#endif
...
#else //basic snap function without timeout limitation
...
static void kxtf9_isr_work_func(struct work_struct *work) 						
{
	 ...
	 kxtf9_accel_service_isr(data->vib_duration);
	 kxtf9_enable_irq();
     //enable_irq(data->irq); 
}
//20101006 taehwan.kim@lge.com To defend tapping from Cable insertion[START_LGE]
static void kxtf9_flip_vibration_work_func(struct work_struct *work) 						
{
	 struct kxtf9_data *data = i2c_get_clientdata(this_client);

	 kxtf9_accel_service_flip(data->vib_duration);
	 kxtf9_enable_irq(); //enable_irq(data->irq); 
     printk("[antispoon] %s irq enable \n",__func__);
}


static void kxtf9_tap_defence_work_func(struct work_struct *work)
{
    struct kxtf9_data *data = i2c_get_clientdata(this_client);

    if (atomic_read(&tap_flag) == 1) {
        atomic_set(&tap_flag, 0);
        hrtimer_start(&data->timer,ktime_set(0,600000000),HRTIMER_MODE_REL);
    }
}
//20101006 taehwan.kim@lge.com To defend tapping from Cable insertion[END_LGE]
#if ENHANCED_SNAP
...
void check_flip_event(int vib_state, int vib_duration)
{
    if (complete_probe == 1)
    {
        struct kxtf9_data *data = i2c_get_clientdata(this_client);
        if (vib_pre_state != vib_state) {
            if (vib_state == 0)
            {
                if (is_flip_enabled())
                {
                    ...
                    printk("antispoon flip_event \n");
                    queue_work(data->flip_vib_wq,&data->flip_in_vibration);
                }
            }
            else if (vib_state == 1) 
            { 
                if (is_flip_enabled())
                {
                    data->vib_duration = vib_duration;
                    printk("[antispoon] %s irq disable \n",__func__);
                    kxtf9_disable_irq();
                }
            }
            vib_pre_state = vib_state;
        }
    }
}   
EXPORT_SYMBOL(check_flip_event);
...
#else //disable tapping event for a instance to prevent tap event from cable or headset insertion shock
//20101006 taehwan.kim@lge.com To defend tapping from Cable insertion[START_LGE]
static enum hrtimer_restart kxtf9_tap_defence_timer_func(struct hrtimer *timer)
 {
 	 struct kxtf9_data *data = container_of(timer, struct kxtf9_data, timer);
...
void instant_pause_tapping(void)
{
    struct kxtf9_data *data = i2c_get_clientdata(this_client);

    queue_work(data->tap_defend_wq, &data->tap_defence_work);
}
EXPORT_SYMBOL(instant_pause_tapping);
//20101006 taehwan.kim@lge.com To defend tapping from Cable insertion [END_LGE]
 /*---------------------------------------------------------------------------
 ...
 static ssize_t kxtf9_tap_onoff_store(struct device *dev,struct device_attribute *attr,const char *buf,size_t count)
 {
	 ...
	 if(tap_onoff_count>0)
	 {
         check_flip_event(1,0); //20101212 taehwan.kim@lge.com enable isr flip event
         kxtf9_accel_enable_tap();
		 atomic_set(&tap_flag, 1);	 
		 kxtf9_enable_irq();
         //KIONIX_ACCEL_read_bytes(KIONIX_ACCEL_I2C_INT_REL, &int_rel, 1); //20101212 taehwan.kim@lge.com clear interrupt register
         //printk("[KXTF9] INT_REL reg = %x \n",int_rel);
	 }
	 else
	 {	 
...
#endif
	 val = simple_strtoul(buf, NULL, 10);
#if defined(CONFIG_MACH_LGE_HUB_REV_A) //Disable_snap_function (Hub don't use snap function //NO_COMPASS_SUPPORT
...
#endif //defined(CONFIG_MACH_LGE_HUB_REV_A) 
	 return count;
...
static ssize_t kxvalue_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
 ...
 static int __init kxtf9_probe(struct i2c_client *client, const struct i2c_device_id *id)
 {
	...
	kxtf9_accel_register_i2c_func();
     ...
     INIT_WORK(&kxtf9->flip_in_vibration,kxtf9_flip_vibration_work_func);
...
#else
//20101006 taehwan.kim@lge.com To defend tapping from Cable insertion[START_LGE]     
     INIT_WORK(&kxtf9->tap_defence_work,kxtf9_tap_defence_work_func);
//20101006 taehwan.kim@lge.com To defend tapping from Cable insertion[END_LGE]     
#endif
	 /*---------------------------------------------------------------------------
		 init. workqueue 
     ...
     kxtf9->flip_vib_wq = create_singlethread_workqueue("kxtf9_flip_vib_wq");
	 if (!kxtf9->isr_wq) {
		 printk("[motion_sensor] couldn't create FLIP queue\n");
		 goto err_accel_flip_wq;
	 }
...
#endif
//20101006 taehwan.kim@lge.com To defend tapping from Cable insertion [START_LGE]
     kxtf9->tap_defend_wq = create_singlethread_workqueue("kxtf9_tap_defence_wq");
     if (!kxtf9->tap_defend_wq) {
         printk("[motion_sensor] couldn't create snap queue\n");
         goto err_accel_tap_defend_wq;
     }
//20101006 taehwan.kim@lge.com To defend tapping from Cable insertion [END_LGE]     
	/*---------------------------------------------------------------------------
		 init. timer
...
#else //disable tapping event for a instance to prevent tap event from cable or headset insertion shock
     kxtf9->timer.function = kxtf9_tap_defence_timer_func;
#endif
	 ...
	 kxtf9->use_irq	= 0;
     kxtf9->vib_duration = 0;
 
	/*---------------------------------------------------------------------------
		power 
	  ---------------------------------------------------------------------------*/  
	  hub_gyro_vio_reg = regulator_get(dev, "vmmc1");
	  ...
	  kxtf9_accel_reg = regulator_get(dev, "vmmc2");
...
err_accel_flip_wq:
   destroy_workqueue(kxtf9->flip_vib_wq);
err_accel_tap_defend_wq:
   destroy_workqueue(kxtf9->tap_defend_wq);
#if ENHANCED_SNAP
err_accel_snap_wq:
...
{
    int ret = 0;
    ret = KIONIX_ACCEL_sleep();
    if (ret == 1) printk("[kxtf_suspend] Failed to set sleep bit of control register in KXTF9 \n");
