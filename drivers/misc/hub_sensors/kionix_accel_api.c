...
#define DEBUG 0
void hub_vibrator_gpio_enable (int enable);
int check_flip_count = 0;
int check_no_flip_count = 0;
//antispoon
...
void KIONIX_ACCEL_PROCESS_screen_rotation(unsigned char tilt_pos_pre, unsigned char tilt_pos_cur)
{
	...
	//if(tilt_pos_pre!=tilt_pos_cur) //20101108 taehwan.kim@lge.com For tunning Flip event 
	{
...
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.07.07] - DEBUG condition is added.
#if DEBUG
...
#endif
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.07.07]- DEBUG condition is added.
		break;
	}
...
/*==================================================================================================
FUNCTION: 		KIONIX_ACCEL_service_flip
...
==================================================================================================*/
int  KIONIX_ACCEL_service_flip(int vib_time)
{
    ...
    int search_number = 8;
    int flip_detect_threshold = 6;
    int not_flip_detect_threshold = 4;
    int report_rotation_count = 1;
    int intial_delay_time = 90;
	// disable accelerometer interrupt first
	KIONIX_ACCEL_disable_interrupt();

	//KIONIX_ACCEL_read_bytes(KIONIX_ACCEL_I2C_STATUS_REG, &status_reg, 1);
    printk("[antispoon] %s VIB_TIME = %d\n ",__func__,vib_time);
#ifndef BIT
#	define BIT(x)	(1<<x)
#endif
	//if(!(status_reg & BIT(4)) ) {
    ...
    if(/*(int_src_reg2 & BIT(0)) &&*/ (is_flip_enabled() )) { // TPS : Screen Rotation
        int num_of_position[6] = {0}, i, j; //20101108 taehwan.kim@lge.com For tunning flip event
        int test_position[40] = {0}, test_index = 0;

//20101108 taehwan.kim@lge.com For tunning flip event [START_LGE]

        if (vib_time >= 1200) 
        {
            search_number = 13;
            flip_detect_threshold = 9;
            not_flip_detect_threshold = 7;
            report_rotation_count = 1;
            intial_delay_time = 130;
        }
        else if (vib_time >= 1000)
        {
            search_number = 11;
            flip_detect_threshold = 8;
            not_flip_detect_threshold = 6;
            report_rotation_count = 1;
            intial_delay_time = 110;
        }
        msleep(intial_delay_time);
        KIONIX_ACCEL_read_bytes(KIONIX_ACCEL_I2C_TILT_POS_PRE, &tilt_pos_pre, 1);

        for (j = 0; j < search_number; j++)
        {
            KIONIX_ACCEL_read_bytes(KIONIX_ACCEL_I2C_TILT_POS_CUR, &tilt_pos_cur, 1);
            for (i = 0; i < 6; i++) if ((tilt_pos_cur >> i) == 1) num_of_position[i]++; 
            test_position[test_index] = tilt_pos_cur; test_index++;
            msleep(20);
        }
        printk("[antispoon] election result [%d %d %d %d %d %d] \n",num_of_position[0],num_of_position[1] 
                ,num_of_position[2] ,num_of_position[3] ,num_of_position[4] ,num_of_position[5]);
        printk("[antispoon] position order ");
        for (i = 0; i < 30; i++) printk("[%d]",test_position[i]);
        printk("\n");
        
        for (i = 0; i < 6; i++) 
        {
            if (num_of_position[i] >= flip_detect_threshold) 
            {
                if (i == 1) 
                {
                    check_no_flip_count = 0;
                    check_flip_count++;
                }
                else
                {
                ...
                printk("check_no_flip_count = %d check_flip_count = %d \n",check_no_flip_count, check_flip_count);
                if ((check_flip_count >= report_rotation_count)
                        ||(check_no_flip_count >= report_rotation_count)) 
                    KIONIX_ACCEL_PROCESS_screen_rotation(tilt_pos_pre, 1<<i);
            }
            else if (num_of_position[i] >= not_flip_detect_threshold)
            {
                if (i != 1)
                {
                    check_flip_count = 0;
                ...
                printk("minor check_no_flip_count = %d check_flip_count = %d \n",check_no_flip_count, check_flip_count);
                if (check_no_flip_count >= report_rotation_count) KIONIX_ACCEL_PROCESS_screen_rotation(tilt_pos_pre, 1<<i);
            }
        }
//20101108 taehwan.kim@lge.com For tunning flip event [END_LGE]     
    }
...
IMPORTANT NOTES:	Do not call this from interrupt context since it accesses i2c.
==================================================================================================*/
int  KIONIX_ACCEL_service_interrupt(int vib_time_value)
{
	unsigned char status_reg=0, int_rel=0;
	unsigned char int_src_reg1, int_src_reg2;
	unsigned char tilt_pos_cur, tilt_pos_pre;
...
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.07.07] - DEBUG condition is added.
    ...
    printk("KXTF9_I2C_INT_SRC_REG2 = %x is_tap_enabled = %d, vib_time_value = %d\n"
            ,int_src_reg2,is_tap_enabled(),vib_time_value);
#endif	
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.07.07]- DEBUG condition is added.
    ...
    if((int_src_reg2 & BIT(0)) && (is_flip_enabled()) && (vib_time_value < 100)) { // TPS : Screen Rotation
        int num_of_position[6] = {0}, i; //20101108 taehwan.kim@lge.com For tunning flip event
        int test_position[20] = {0}, test_index = 0;
...
/*
        ...
        for (i = 0; i < 20; i++) printk("[%d]",test_position[i]);
        printk("\n");
*/        
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.07.07]- DEBUG condition is added.
        
        for (i = 0; i < 6; i++) if (num_of_position[i] >= 2) KIONIX_ACCEL_PROCESS_screen_rotation(tilt_pos_pre, 1<<i); 
//20101108 taehwan.kim@lge.com For tunning flip event [END_LGE]     
    }
