...
//--[[ LGE_UBIQUIX_MODIFIED_START : nullcha@mnbt.co.kr - change for TI wl127x
static int wl127x_bt_rfkill_set_power(void *data, bool blocked)
{
	...
	printk(KERN_ERR "wl127x_bt_rfkill_set_power (%d)", blocked);
...
static int wl127x_fm_rfkill_set_power(void *data, bool blocked)
{
	...
	printk(KERN_ERR "wl127x_fm_rfkill_set_power (%d)", blocked);
...
static int wl127x_rfkill_probe(struct platform_device *pdev)
{
	...
	printk(KERN_ERR "wl127x_rfkill_probe\n");
...
//--]] LGE_UBIQUIX_MODIFIED_END : nullcha@mnbt.co.kr - change for TI wl127x
