...
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.07.14] - For kzalloc / kfree
#include <linux/slab.h>
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.07.14]- For kzalloc / kfree
...
//	if (lcd_off_boot) return 0;
 ...
 void kxtf9_enable_irq(void)
 {
	 ...
	 if(data->use_irq == 0)
	 {
//For_Resume_Speed		 printk("[kxtf9_enable_irq] data->use_irq == 0\n");
		 enable_irq(data->irq);
		 data->use_irq = 1;
	 }
 }
 void kxtf9_disable_irq(void)
 {
	 ...
	 if(data->use_irq == 1)
	 {
//For_Resume_Speed		 printk("[kxtf9_disable_irq] hub_motion_dev.use_irq  == 1\n");
...
static ssize_t kxvalue_store(struct device *dev,struct device_attribute *attr,const char *buf, size_t count)
{
 ...
 static int __init kxtf9_probe(struct i2c_client *client, const struct i2c_device_id *id)
 {
	...
	printk("## SHYUN ## kxtf9_probe [NORMAL OUT]\n");
