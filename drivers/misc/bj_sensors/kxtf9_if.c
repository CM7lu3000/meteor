...
void kxtf9_accel_send_screen_rotation_event(int value, int pre_value)
{
   ...
   if(status & KXTF9_FLIP_ENABLE)
   {
	...
	//if((value == 0x01) || (value == 0x02))
	{
		if(call_once == 1)
		{
			flip_pre_pre_state = value;
			call_once = 0;
		}

//		if(flip_pre_pre_state != value)
		{
		       if(value == 0x01)
		       {
				flip_data= ACCEL_FLIP_UPSIDE_DOWN; //ACCEL_FLIP_DOWNSIDE_UP;
...
int  kxtf9_accel_init(void)
{
	...
	int res=0;
//	unsigned char   	rwbuf[2]; 

	/* Need 50ms until the RAM load is finished after Power-up	*/
    msleep(50);

//	mpu3050_i2c_read(0x3D,&rwbuf[0], 1);
//
//	if(rwbuf[0] & 0x20) //if 1, bypass off
//		return res;

    KIONIX_ACCEL_read_bytes(KIONIX_ACCEL_I2C_WHO_AM_I, &value, 1);
//For_Resume_Speed    printk("[%s:%d] [MOTION] Device ID (0x%02x) \n", __FUNCTION__, __LINE__, value);
    KIONIX_ACCEL_reset();
    
    msleep(200);
    KIONIX_ACCEL_read_bytes(KIONIX_ACCEL_I2C_WHO_AM_I, &value, 1);
//For_Resume_Speed   printk("[%s:%d] [MOTION] Device ID 2nd try (0x%02x) \n", __FUNCTION__, __LINE__, value);
...
{
	int res = 0;
	unsigned char old_vco_trim/*, new_vco_trim*/;
...
//For_Resume_Speed    printk("------------------------------> YJ : old_vco_trim = 0x%x  new_vco_trim = 0x%x\n", old_vco_trim, new_vco_trim);
