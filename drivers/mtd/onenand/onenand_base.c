...
/* LGE_CHANGE [VX20000:FW:greendrm@lge.com] 2009-05-12 */
#ifdef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
/**
 *  onenand_oob_128 - oob info for Onenand with SLC 4KB page
 */
...
#endif/* CONFIG_LG_FW_MTD_ONENAND_SLC_4KB */
...
static int onenand_page_address(int page, int sector)
{
	...
	/* LGE_CHANGE [VX20000:FW:greendrm@lge.com] 2009-05-14, extension for fsa */
#ifdef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
	fsa = ((sector & ONENAND_FSA_MSB_MASK) << ONENAND_FSA_MSB_SHIFT)
		| (sector & ONENAND_FSA_MASK);
#else
...
static int onenand_command(struct mtd_info *mtd, int cmd, loff_t addr, size_t len)
{
	...
	if (cmd == ONENAND_CMD_BUFFERRAM) {
		...
		/* LGE_CHANGE [VX20000:FW:greendrm@lge.com] 2009-05-13, support single buffer */
#ifdef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
		if (ONENAND_IS_2PLANE(this) || ONENAND_IS_1BUFFER(this))
#else
...
/* LGE_CHANGE [VX20000:FW:greendrm@lge.com] 2009-05-13, support single buffer */
#ifdef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
			if (ONENAND_IS_1BUFFER(this) || ONENAND_IS_MLC(this))
#else
...
static inline int onenand_read_ecc(struct onenand_chip *this)
{
...
#ifndef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
	...
	for (i = 0; i < 4; i++) {
		...
		if (ecc & ONENAND_ECC_4BIT_UNCORRECTABLE ||
				ecc & FLEXONENAND_UNCORRECTABLE_ERROR)
#else
				...
				struct mtd_oob_ops *ops)
{
	...
	if (column + thislen > writesize)
 	...
 	while (!ret) {
 		...
 		from += thislen;
		/* LGE_CHANGE [VX20000:FW:greendrm@lge.com] 2009-05-13, support for single bufferram */
#ifdef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
 		if (!ONENAND_IS_1BUFFER(this) && (read + thislen < len)) {
#else
 		if (read + thislen < len) {
#endif
			this->command(mtd, ONENAND_CMD_READ, from, writesize);
...
static void onenand_check_features(struct mtd_info *mtd)
{
	...
	case ONENAND_DEVICE_DENSITY_4Gb:
		...
		if (ONENAND_IS_DDP(this))
			...
			this->options |= ONENAND_HAS_4BIT_ECC;
#else
		if (ONENAND_IS_DDP(this))
			this->options |= ONENAND_HAS_2PLANE;
		else
	...
	/* LGE_CHANGE [VX20000:FW:greendrm@lge.com] 2009-05-22, info */
#ifdef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
	printk("OneNAND %s, ECC %d bit(s), BUFFER %d\n", 
			ONENAND_IS_DDP(this)? "DDP" : "MONO",
			ONENAND_IS_4BIT_ECC(this)? 4 : 1,
			ONENAND_IS_1BUFFER(this)? 1 : 2);
#endif
...
static int onenand_probe(struct mtd_info *mtd)
{
	...
	int density;
	int syscfg;
	/* LGE_CHAGNE [VX20000:FW:greendrm@lge.com] 2009-05-14, extension information*/
#ifdef CONFIG_PRODUCT_LGE_HUB
	int buf_amount;
#endif
#ifdef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
	int databuf_size;
...
#ifndef CONFIG_PRODUCT_LGE_HUB /* 20101002 jugwan.eom@lge.com OMAP GPMC was already configured to sync-mode.
			       Clear Sync. lead to failure to access OneNAND */
...
#ifndef CONFIG_PRODUCT_LGE_HUB /* 20101002 jugwan.eom@lge.com bram_* variables are not used. Refer to the above codes. */
...
#ifdef CONFIG_PRODUCT_LGE_HUB
	/* LGE_CHAGNE_S [VX20000:FW:greendrm@lge.com] 2009-05-13, extension information*/
	buf_amount = this->read_word(this->base + ONENAND_REG_NUM_BUFFERS);
	this->databuf_amount = buf_amount >> 8;
	/* LGE_CHAGNE_E [VX20000:FW:greendrm@lge.com] 2009-05-13, extension information*/
#endif
	...
	/* LGE_CHANGE [VX20000:FW:greendrm@lge.com] 2009-05-14, fix the writesize */
#ifdef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
	databuf_size = this->read_word(this->base + ONENAND_REG_DATA_BUFFER_SIZE); /* in 2bytes */
	mtd->writesize = (databuf_size << 1) / this->databuf_amount;
#else
...
int onenand_scan(struct mtd_info *mtd, int maxchips)
{
	...
	case 128:
		...
		/* LGE_CHANGE [VX20000:FW:greendrm@lge.com] 2009-05-14, support for oobsize 128 */
#ifdef CONFIG_LG_FW_MTD_ONENAND_SLC_4KB
		mtd->subpage_sft = 3;
#else
		mtd->subpage_sft = 0;
...
/* S[, 20111024, mschung@ubiquix.com, Is USB CP retain mode? */
	{
		size_t retlen = 0;
		u_char buf[256];
		...
		memset(buf, 0x00, 256);
		onenand_read(mtd, 0x3FD80000 + 0x1000, 256, &retlen, buf);

		if (strncmp(buf, "boot-cpusb", 10) ==0) {
			hub_cp_usb_mode = 1;
		}
		else {
			hub_cp_usb_mode = 0;
...
/* E], 20111024, mschung@ubiquix.com, Is USB CP retain mode? */
