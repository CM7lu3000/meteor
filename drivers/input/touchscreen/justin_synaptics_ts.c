...
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.07.12] - for kzalloc
#include <linux/slab.h>
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.07.12]- for kzalloc
#include <mach/gpio.h>
...
//--[[ LGE_UBIQUIX_MODIFIED_START : shyun@ubiquix.com [2011.07.12] - JUSTIN Define modified.
//#ifdef CONFIG_MACH_LGE_JUSTIN
#ifdef CONFIG_PRODUCT_LGE_JUSTIN
//--]] LGE_UBIQUIX_MODIFIED_END : shyun@ubiquix.com [2011.07.12]- JUSTIN Define modified.
...
#define SYNAPTICS_FW_FORCE_UPGRADE	0
	...
	for(ts_cnt = 0; ts_cnt < SYNAPTICS_FINGER_MAX; ts_cnt++)
	{
		...
		if(curr_ts_data.touch_status[ts_cnt])
		{
			...
			else // multi-finger
			{
				...
				//printk(KERN_INFO"[touch]*multifinger* check =%d finger_count=%d  ts_i=%d (X, Y) = (%d, %d), z = %d, w = %d\n",check,finger_count, ts_cnt, curr_ts_data.X_position[ts_cnt], curr_ts_data.Y_position[ts_cnt], curr_ts_data.pressure[ts_cnt], curr_ts_data.width[ts_cnt]);
...
static irqreturn_t synaptics_ts_irq_handler(int irq, void *dev_id)
{
	...
	//ts_printk("LGE: synaptics_ts_irq_handler\n");
...
static int synaptics_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if( gpio_request(59, "touch_gpio") < 0 )
		printk("[synaptics_ts_probe] touch_gpio request error!\n");
	if ( gpio_request(162, "touch_gpio2") < 0 )
		printk("[synaptics_ts_probe] touch_gpio2 request error!\n");
	...
	ts->input_dev->name = "justin_synaptics_touch";
	...
	ts_printk(KERN_INFO "synaptics_ts_probe: max_x %d, max_y %d\n", max_x, max_y);
	...
	printk(KERN_WARNING"########## irq [%d], irqflags[0x%x]\n", client->irq, IRQF_TRIGGER_FALLING);
	...
	ts_printk(KERN_INFO "synaptics_ts_probe: Start touchscreen %s in %s mode\n", ts->input_dev->name, ts->use_irq ? "interrupt" : "polling");
	return 0;
