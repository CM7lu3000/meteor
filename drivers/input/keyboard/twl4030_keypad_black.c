...
// 20110331 youngmin.kim@lge.com, KEY resume check. for LED [START_LGE]
extern void resumekey_pressed();
// 20110331 youngmin.kim@lge.com, KEY resume check. for LED [END_LGE]
...
// 20110424 youngmin.kim@lge.com, KEY irq enable/disable [START_LGE]
static struct twl4030_keypad *twl_key = NULL;
// 20110424 youngmin.kim@lge.com, KEY irq enable/disable [END_LGE]
/*----------------------------------------------------------------------*/
...
#if 0
				resumekey_pressed();
				printk("[shyun] KEYKPJPCOMMA Key pressed\n");
				}
#else
				printk("[shyun] KEYKPJPCOMMA Key pressed\n");
			}
...
#else
     //modem crash reset disable - inho.oh@lge.com 20110504[START]
      // must be check to modem crash in booting sequence by kweop
      input_report_key(kp->input, KEY_PROG3, 1);
      mdelay(10);
      input_report_key(kp->input, KEY_PROG3, 0);
      input_sync(kp->input);
	 //modem crash reset disable - inho.oh@lge.com 20110504[END]
...
// 20100727 jh.koo@lge.com for CP reset Pop up [END_LGE]	
	...
	printk("[shyun] twl4030_kp_probe [IN]\n");
...
// 20100831 jh.koo@lge.com for TEST MODE [END_LGE]
	...
	printk("[shyun] twl4030_kp_probe [NORMAL OUT]\n");
...
static int __init twl4030_kp_init(void)
{
	...
	printk("[shyun] twl4030_kp_init [IN]\n");

	ret = platform_driver_register(&twl4030_kp_driver);

	printk("[shyun] twl4030_kp_init [OUT ret = %d]\n",ret);
