...
int ds_update_time_counter(void){
	...
	/* Busy */
		...
		if(ds_counter.busy_fse_usec_fra_fp12 >= 0x1000){
			ds_counter.busy_fse_usec += (lc_usec_interval_fse + 1);
		...
		switch(ds_status.cpu_mode){
			...
			case DS_CPU_MODE_TASK:
				...
				if(ds_counter.busy_task_fse_usec_fra_fp12 >= 0x1000){
					ds_counter.busy_task_fse_usec += (lc_usec_interval_fse + 1);
			...
			case DS_CPU_MODE_SCHEDULE:
				...
				if(ds_counter.busy_schedule_fse_usec_fra_fp12 >= 0x1000){
					ds_counter.busy_schedule_fse_usec += (lc_usec_interval_fse + 1);
			...
			case DS_CPU_MODE_DVS_SUITE:
				...
				if(ds_counter.busy_dvs_suite_fse_usec_fra_fp12 >= 0x1000){
					ds_counter.busy_dvs_suite_fse_usec += (lc_usec_interval_fse + 1);
					...
					unsigned long speedup_interval)
{
	...
	/* (1) Update stat->time_usec_interval, 
			...
			stat->time_usec_work_fse_lasting.
	 */
	...
	//if(stat->time_usec_interval > 1000000) stat->time_usec_interval = 1000000;
	...
	if(stat->flag_in_busy_half == 1){
		...
		//if(stat->time_usec_work_fse > 1000000) stat->time_usec_work_fse = 1000000;

		stat->time_usec_work_fse_lasting += lc_time_usec_work_fse_inc;
		//if(stat->time_usec_work_fse_lasting > 1000000) stat->time_usec_work_fse_lasting = 1000000;
	...
	if(ds_parameter.entry_type == DS_ENTRY_SWITCH_TO){
			...
			ds_parameter.next_p->static_prio <= target_static_prio)
		{
			...
			/* Until all the stat->interval_window_array[] is filled, 
				...
				time_usec_interval and time_usec_work_fse to keep the number of
				intervals that are used for the calculation interval_window_size.
			 */
			...
			stat->time_usec_interval_in_window +=
				stat->interval_window_array[stat->interval_window_index].time_usec_interval;
			//if(stat->time_usec_interval_in_window > 1000000) stat->time_usec_interval_in_window = 1000000;

			stat->time_usec_work_fse_in_window +=
				stat->interval_window_array[stat->interval_window_index].time_usec_work_fse;
			//if(stat->time_usec_work_fse_in_window > 1000000) stat->time_usec_work_fse_in_window = 1000000;
			...
			if(stat->time_usec_interval_in_window >= interval_window_length){
#if 1	// Moving average strategy.
				/* Calculate current interval window's utilization */
				...
				stat->cpu_op_index =
					ds_get_next_low_cpu_op_index(
						stat->utilization_int_ulong,
						stat->utilization_fra_fp12);

#if 0
printk(KERN_WARNING "%-5d %-8lu %-8lu %-8lu %-8lu 0x%lx 0x%-5lx %-8u\n", 
ds_parameter.prev_p->pid,
ds_counter.busy_fse_usec,
...
int ds_do_dvs_gpschedvs(unsigned int *target_cpu_op_index){
	...
	if(ds_status.flag_do_post_early_suspend == 0){
		...
		if(ds_do_dvs_aidvs(&lc_target_cpu_op_index_highest,
							 ...
							 DS_HRT_STATIC_PRIO,
							 DS_GPSCHEDVS_L0_INTERVAL_WINDOW_SIZE,
							 DS_GPSCHEDVS_L0_INTERVAL_WINDOW_LENGTH,
		...
		if(ds_do_dvs_aidvs(&lc_target_cpu_op_index_aidvs_l1,
							 ...
							 DS_GPSCHEDVS_L1_INTERVAL_WINDOW_SIZE,
							 DS_GPSCHEDVS_L1_INTERVAL_WINDOW_LENGTH,
		...
		if(ds_do_dvs_aidvs(&lc_target_cpu_op_index_aidvs_l2,
							 ...
							 DS_GPSCHEDVS_L2_INTERVAL_WINDOW_SIZE,
							 DS_GPSCHEDVS_L2_INTERVAL_WINDOW_LENGTH,
		...
		if(ds_configuration.gpschedvs_strategy == 0){
			if(ds_do_dvs_aidvs(&lc_target_cpu_op_index_aidvs_l3,
								 ...
								 DS_NRT_STATIC_PRIO,
								 DS_GPSCHEDVS_L3_INTERVAL_WINDOW_SIZE,
								 DS_GPSCHEDVS_L3_INTERVAL_WINDOW_LENGTH,
...
int ds_initialize_dvs_suite(int cpu_mode){
	...
	for(i=0;i<DS_GPSCHEDVS_L3_INTERVAL_WINDOW_SIZE;i++){
			...
			if(ds_parameter.next_p->pid != 0){
				...
				switch(*(ds_status.tg_owner_comm[ds_parameter.next_p->pid]+0)){
...
#endif
#if 0	// bluetoothd. Bluetooth reltated [daemon process]. HRT.
						if(
...
#endif
#if 0	// mediaserver. Android system service [daemon process]. O 4 touch.
						if(
							*(ds_status.tg_owner_comm[ds_parameter.next_p->pid]+1) == 'e' &&
							*(ds_status.tg_owner_comm[ds_parameter.next_p->pid]+2) == 'd' &&
							*(ds_status.tg_owner_comm[ds_parameter.next_p->pid]+3) == 'i' &&
					...
					case '/':
						break;
					case '.':
#if 1	// android.process.lghome. [application]. O 4 touch.
#if 1
						if(
							*(ds_status.tg_owner_comm[ds_parameter.next_p->pid]+1) == 'p' &&
							*(ds_status.tg_owner_comm[ds_parameter.next_p->pid]+2) == 'r' &&
...
#if 0
						if(
							*(ds_parameter.next_p->comm+1) == 'p' &&
							*(ds_parameter.next_p->comm+2) == 'r' &&
							*(ds_parameter.next_p->comm+3) == 'o' &&
							*(ds_parameter.next_p->comm+4) == 'c' &&
							*(ds_parameter.next_p->comm+5) == 'e' &&
							*(ds_parameter.next_p->comm+6) == 's' &&
							*(ds_parameter.next_p->comm+7) == 's' &&
							*(ds_parameter.next_p->comm+8) == '.' &&
							*(ds_parameter.next_p->comm+9) == 'l' &&
							*(ds_parameter.next_p->comm+10) == 'g' &&
							*(ds_parameter.next_p->comm+11) == 'h' &&
							*(ds_parameter.next_p->comm+12) == 'o' &&
							*(ds_parameter.next_p->comm+13) == 'm' &&
							*(ds_parameter.next_p->comm+14) == 'e'
						)
#endif
						{
							new_type = DS_SRT_DAEMON_TASK;
...
DS_UNKNOWN_TASK_NAME:
				...
				if(old_type == DS_SRT_IA_TASK || old_type == DS_SRT_CM_TASK)
				{
					/* If DS_SRT_UIE_TIMEOUT is over */
					...
					/* If DS_SRT_UIE_TIMEOUT is not over yet */
					else 
						new_type = DS_SRT_IA_TASK;
				}
				else{
		...
		ds_status.flag_correct_cpu_op_update_path = 1;
 		...
 		switch(ds_status.cpu_op_index){
 			...
 			case DS_CPU_OP_INDEX_1:
 				break;
 			case DS_CPU_OP_INDEX_2:
 				if(ds_status.target_cpu_op_index == DS_CPU_OP_INDEX_0)
 					ds_status.target_cpu_op_index = DS_CPU_OP_INDEX_1;
 				break;
 			case DS_CPU_OP_INDEX_3:
 				if(ds_status.target_cpu_op_index == DS_CPU_OP_INDEX_0 ||
 					ds_status.target_cpu_op_index == DS_CPU_OP_INDEX_1)
 					ds_status.target_cpu_op_index = DS_CPU_OP_INDEX_2;
 				break;
 		}
#endif
