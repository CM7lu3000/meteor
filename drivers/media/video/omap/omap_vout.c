...
//--[[ LGE_UBIQUIX_MODIFIED_START : hycho@ubiquix.com [2011.08.08] - 720p contents -> 480p mode play problem
#if defined(CONFIG_PRODUCT_LGE_HUB) || defined(CONFIG_PRODUCT_LGE_JUSTIN)
					|| 	(is_hdmi_enabled()&&GET_RESOLUTION()==_720X480P&&(vout->pix.height * vout->pix.width > VID_MAX_WIDTH * 480))
#endif					
//--]] LGE_UBIQUIX_MODIFIED_END : hycho@ubiquix.com [2011.08.08] - 720p contents -> 480p mode play problem
						){
	...
	if (cpu_is_omap34xx())
	{
//--[[ LGE_UBIQUIX_MODIFIED_START : hycho@ubiquix.com [2011.08.12] - Added hdmi 720p play
#if defined(CONFIG_PRODUCT_LGE_HUB) || defined(CONFIG_PRODUCT_LGE_JUSTIN)		
		extern int is_hdmi_enabled(void);
#endif
...
//			&& !is_hdmi_enabled()
#endif			
//--]] LGE_UBIQUIX_MODIFIED_END : hycho@ubiquix.com [2011.08.12] - Added hdmi 720p play
			)
		{
				//printk(KERN_ERR "[yhlee] <%s> CheckThis111\n", __func__);
				vout->use_isp_rsz_for_downscale = 1;
		}
		else 
		{
			//printk(KERN_ERR "[yhlee] <%s> CheckThis222\n", __func__);
...
#if defined(CONFIG_PRODUCT_LGE_HUB) || defined(CONFIG_PRODUCT_LGE_JUSTIN) //== 2011.06.08 === hycho@ubiquix.com froyo code merge
		if (!(irqstatus & (DISPC_IRQ_VSYNC | DISPC_IRQ_FRAMEDONE)))
			goto vout_isr_err;
		break;
#else
