...
tmdlHdmiTxVideoInfo_t videoInfoListTx = {
    False,
    {TMDL_HDMITX_VFMT_03_720x480p_60Hz, TMDL_HDMITX_VINMODE_YUV422, TMDL_HDMITX_SYNCSRC_EXT_VS, TMDL_HDMITX_PIXRATE_SINGLE},
...
//	printk(KERN_INFO "[hycho9] -- tmdlHdmiTxOpenM\n");		
    ...
    if (errCode != TM_OK)
    {
//		printk(KERN_INFO "[hycho10] -- tmdlHdmiTxOpenM\n");
        /* Init failed */
...
//		printk(KERN_INFO "[hycho3] -- tmdlHdmiTxOpenM\n");
    ...
    /* Release the sempahore */
    RETIF( (errCodeSem = tmdlHdmiTxIWSemaphoreV(dlHdmiTxItSemaphore[unit]) ) != TM_OK, errCodeSem)
//	printk(KERN_INFO "[hycho6] -- tmdlHdmiTxOpenM\n");
    return TM_OK;
}
...
)
{
    ...
    errCode = tmbslHdmiTxSetVideoPortConfig(instance,
        pEnaVideoPortTable, pGndVideoPortTable);
    RETIF_SEM(dlHdmiTxItSemaphore[instance], (errCode != TM_OK) && (errCode != TMBSL_ERR_HDMI_NOT_SUPPORTED), errCode)

    errCode = tmbslHdmiTxVideoInSetMapping(instance, 
        pSwapTable, pMirrorTable);
    RETIF_SEM(dlHdmiTxItSemaphore[instance], (errCode != TM_OK) && (errCode != TMBSL_ERR_HDMI_NOT_SUPPORTED), errCode)
    
    /* Set fine image position */
    RETIF_SEM(dlHdmiTxItSemaphore[instance],
        (errCode = tmbslHdmiTxVideoInSetFine(instance, 
        spSync, HDMITX_PIXTOGL_NO_ACTION) ) != TM_OK, errCode)

    /* Set input blanking */
    errCode = tmbslHdmiTxVideoInSetBlanking(instance, 
    ...
    RETIF_SEM(dlHdmiTxItSemaphore[instance],
        ...
        (tmbslHdmiTxVinMode_t)videoInputConfig.mode, (tmbslHdmiTxVidFmt_t)videoOutputConfig.format, pixelEdge, 
    ...
    RETIF_SEM(dlHdmiTxItSemaphore[instance],
        (errCode = tmbslHdmiTxVideoSetInOut(instance, 
        (tmbslHdmiTxVidFmt_t)videoInputConfig.format, HDMITX_SCAMODE_AUTO, (tmbslHdmiTxVidFmt_t)videoOutputConfig.format,
    ...
    if (dlHdmiTxGetReflineRefpix(videoInputConfig.format, videoInputConfig.mode, videoOutputConfig.format, 
    ...
    else
    {
        ...
        RETIF_SEM(dlHdmiTxItSemaphore[instance],
			...
			(tmbslHdmiTxVinMode_t)videoInputConfig.mode) ) != TM_OK, errCode)
    }
    ...
    if ((errCode == TM_OK) && (scalerMode == HDMITX_SCAMODE_ON))
    {
                ...
                || (videoOutputConfig.format == TMDL_HDMITX_VFMT_20_1920x1080i_50Hz))
        {
            ...
            if ((kVfmtToShortFmt_TV[videoInputConfig.format] == TV_480p_60Hz)
            ||  (kVfmtToShortFmt_TV[videoInputConfig.format] == TV_576p_50Hz))
            {
...
#ifndef FORMAT_PC
    contentVif.pictureAspectRatio = kVfmtToAspect_TV[voutFmt];
#else /* FORMAT_PC */
    if ((voutFmt >= TMDL_HDMITX_VFMT_PC_MIN)
        && (voutFmt <= TMDL_HDMITX_VFMT_PC_MAX))
    {
        contentVif.pictureAspectRatio = kVfmtToAspect_TV[TMDL_HDMITX_VFMT_TV_MAX + (voutFmt - TMDL_HDMITX_VFMT_PC_MIN) + 1];
    }
    else 
	    contentVif.pictureAspectRatio = kVfmtToAspect_TV[voutFmt];
#endif /* FORMAT_PC */
