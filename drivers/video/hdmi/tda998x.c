...
int GET_RESOLUTION()
{
	return saving_for_resolution->tda.setio.video_out.format;
}
...
static struct omap_video_timings video_1280x720at60Hz_panel_timings = {
   ...
   .y_res          = 720,
   .pixel_clock    = 74250,
//#ifdef ZOOMII_PATCH //changhoony.lee@lge.com
//   .hfp            = 70,
//   .hbp            = 260,
//   .vfp            = 0,
//   .vbp            = 25,
...
void show_video(tda_instance *this) {
   ...
   if (this->tda.rx_device_active) {
      if (this->tda.hot_plug_detect == TMDL_HDMITX_HOTPLUG_ACTIVE) {
         if (this->tda.power == tmPowerOn) {
            if (this->tda.src_address != 0xFFFF) {
//printk(KERN_INFO " %s this->tda.setio.audio_in.format = %d\n\n", __func__, this->tda.setio.audio_in.format);
...
//         printk(KERN_INFO "/!\\ CAUTION /!\\ sink is not HDMI but %s\n",tda_spy_sink(this->tda.setio.sink));
   ...
   case TMDL_HDMITX_HPD_ACTIVE: 
      ...
      show_video(this);
//	  printk(KERN_INFO " %s TMDL_HDMITX_HPD_ACTIVE\n\n", __func__); 	
...
printk(KERN_INFO " \n\nsaving_for_resolution->tda.setio.video_out.format = %d\n\n", saving_for_resolution->tda.setio.video_out.format);
...
static int hdmi_panel_probe(struct omap_dss_device *dssdev)
{
   ...
   // changhoony.lee@lge.com 2010-4-5
...
//   dssdev->panel.config = OMAP_DSS_LCD_TFT | OMAP_DSS_LCD_ONOFF | OMAP_DSS_LCD_RF | OMAP_DSS_LCD_IPC;
   ...
   dssdev->set_timings(dssdev, &dssdev->panel.timings);

   printk(KERN_INFO "\nhdmi_panel_enable\ndssdev->panel.timings =%d\n", dssdev->panel.timings.y_res);


//2010-5-26 changhoony.lee@lge.com 
//   if(dssdev->get_device_detect(dssdev) == false)
//   {
//	LOG(KERN_INFO,"## HDMI platform_enable\n");
//
...
static int this_cdev_ioctl(struct inode *pInode, struct file *pFile, unsigned int cmd, unsigned long arg)
{
   ...
   printk(KERN_ERR "[hycho] _IOC_DIR:%d\n",_IOC_DIR(cmd));
   switch ( _IOC_NR(cmd) )
      {
...
//printk(KERN_INFO " %s this->tda.setio.audio_in.format = %d\n\n", __func__, this->tda.setio.audio_in.format);
      ...
      case TDA_GET_HDCP_STATUS_CMD:
		...
		case TDA_GET_CABLE_STATUS_CMD:
			{
				printk(KERN_INFO "hdmitx:%s:GET_CABLE_STATUS IOCTL number: %x\n",__func__,cmd);
				tmdlHdmiTxGetHPDStatus(this->tda.instance,&this->tda.hot_plug_detect);
//	   			show_video(this); /* FIXME : mind usecases without HPD and RX_SEND... */				
				printk(KERN_INFO " %s TDA_GET_CABLE_STATUS_CMD = %d\n\n", __func__, this->tda.hot_plug_detect);	  
...
//				printk(KERN_INFO "\nHDMI_INIT_MODE = %d", get_play_mode);
		...
		case TDA_GET_CURRENT_RES:
			{
//				printk(KERN_INFO "hdmitx:%s:GET_CURRENT_RES IOCTL number: %x\n",__func__,cmd);				
