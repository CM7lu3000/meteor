...
u8 lcd_frame[][21] = {
	{LONG_CMD_MIPI, 0x29,0x03,0xF1,0x5A,0x5A,}, //level2 command control test key2
	{LONG_CMD_MIPI,0x29,0x12,0xF7,0x00,0x01,0x00,0xF2,0x08,0x00,0x08,0x20,0x08,0x07,0x1D,0x00,0x03,0x00,0x4B,0x00,0x8C,},
	{LONG_CMD_MIPI, 0x29,0x03,0xF1,0xA5,0xA5,}, //level2 command control test key2
	{END_OF_COMMAND,},
};
...
u8 lcd_gamma[][50] = {
	{LONG_CMD_MIPI, 0x29,0x03,0xF1,0x5A,0x5A,}, //level2 command control test key2
	{LONG_CMD_MIPI, 0x29,0x0A,0xF2,0x00,0x00,0xAA,0xAA,0x57,0x57,0x00,0x00,0x40,}, //level2 command control test key2
	{LONG_CMD_MIPI, 0x29,0x2E,0xFA,0x00,0x39,0x32,0x16,0x1B,0x1B,0x1B,0x11,0x08,0x1C,0x0A,0x09,0x0E,0x07,0x01,
		0x00,0x39,0x31,0x15,0x1A,0x1A,0x1A,0x10,0x06,0x1A,0x08,0x07,0x0C,0x05,0x00,0x05,0x39,0x33,0x17,0x1C,
		0x1D,0x1D,0x13,0x0B,0x20,0x10,0x11,0x18,0x10,0x08,},//positive gamma control //46 data
	{LONG_CMD_MIPI, 0x29,0x2E,0xFB,0x00,0x3F,0x3E,0x37,0x50,0x54,0x53,0x41,0x35,0x4A,0x40,0x3F,0x3F,0x23,0x06,
		0x00,0x3F,0x3F,0x39,0x52,0x56,0x55,0x43,0x37,0x4B,0x41,0x40,0x40,0x24,0x07,0x00,0x3A,0x36,0x2F,0x47,
		0x4D,0x4E,0x3D,0x32,0x48,0x3F,0x3D,0x3F,0x23,0x06,},//nagative gamma control //46 data
	{LONG_CMD_MIPI, 0x29,0x03,0xF1,0xA5,0xA5,}, //level2 command control test key2
	{END_OF_COMMAND,},
};
...
/* S, 20110813, mschung@ubiquix.com, Debug playing sound(music) distortion(blinking) at LCD resume. Delay value change from 20ms to 1ms. */
...
//	mdelay(1);
	...
	gpio_set_value(td->gpio_lcd_reset_n, 1);
	mdelay(1);
/* E, 20110813, mschung@ubiquix.com, Debug playing sound(music) distortion(blinking) at LCD resume. Delay value change from 20ms to 1ms. */
}

/* 20110801, mschung@ubiquix.com, LCD should off at suspend for adequate sleep current. [UB_START] */
static void hub_hw_off(struct omap_dss_device *dssdev)
{
	struct hub_data *td = dev_get_drvdata(&dssdev->dev);

	gpio_set_value(td->gpio_lcd_reset_n, 0);
//	gpio_set_value(td->gpio_lcd_cs, 0); /* 20110801, mschung@ubiquix.com, DEL. Debug LCD_CS line I/O setting. Affecting sleep current. */
}
/* 20110801, mschung@ubiquix.com, LCD should off at suspend for adequate sleep current. [UB_END] */
...
#if 1 /* 20110801, mschung@ubiquix.com, Debug LCD_CS line I/O setting. Affecting sleep current. */
	gpio_direction_output(td->gpio_lcd_cs, 0);
#else
...
static int hub_power_on(struct omap_dss_device *dssdev)
{
	...
	if (lcd_boot_status== 1)  //kyungtae.oh
		lcd_boot_status = 0;
	else
		hub_hw_reset(dssdev);
	...
	for (i = 0; lcd_command_for_mipi[i][0] != END_OF_COMMAND; i++) {
		dsi_vc_dcs_write(ix,TCH,
				&lcd_command_for_mipi[i][3], /* data */
				lcd_command_for_mipi[i][2]); /* lenth */
	}
	...
	read_status_reg(ix, "after init");

	for (i = 0; lcd_frame[i][0] != END_OF_COMMAND; i++) {
			dsi_vc_dcs_write(0,TCH,                                           
					&lcd_frame[i][3], /* data */
					lcd_frame[i][2]); /* lenth */
	}

	for (i = 0; lcd_gamma[i][0] != END_OF_COMMAND; i++) {
		                dsi_vc_dcs_write(0,TCH,                   
						&lcd_gamma[i][3], /* data */
						lcd_gamma[i][2]); /* lenth */
	}

	read_status_reg(ix, "after sleep out");

	if (r)
		goto err;

	hub_dcs_write_1(ix, DCS_CTRL_DISPLAY, (1<<2) | (1<<5)); /* BL | BCTRL */

	hub_dcs_write_1(ix, DCS_PIXEL_FORMAT, 0x7); /* 24bit/pixel */
...
#define _LCD_LEN()	(tear_scanline[1])
		...
		dsi_vc_write(ix, TCH, _LCD_CMD(), _LCD_DAT(), _LCD_LEN());
		r = hub_dcs_write_1(ix, DCS_TEAR_ON, 0);
