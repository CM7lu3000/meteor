...
//bjhyun
printk(KERN_INFO "omapfb_open called\n");
...
void suspend(struct early_suspend *h)
{
	...
	// LGE_DOM_UPDATE_S hayun.kim 2010/09/09 {
	{
		extern int broadcast_tdmb_is_on(void);
		if ( broadcast_tdmb_is_on() )
		{
			printk("omapfb_suspend is called but return because tdmb mode is on\n");
			return;
		}
	}
	// LGE_DOM_UPDATE_E hayun.kim 2010/09/09 }
...
/* S, 20110813, mschung@ubiquix.com, Debug LCD distortion(blinking) at LCD resume. */
	...
	extern int AKECS_temperature_value(void);
	if(AKECS_temperature_value()<-5)
		msleep(270);
	else
		msleep(50);
#endif
