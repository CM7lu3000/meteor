...
#define CONFIG_GFX_HDMI /* BRAD */	// == 2011.06.12 === hycho@ubiquix.com  
#endif
		...
		const char *buf, size_t size)
{
	...
	if (dssdev) {
// == 2011.06.12 === hycho@ubiquix.com START	
#ifdef CONFIG_GFX_HDMI /* BRAD */
		extern void configure_overlay_size(struct omap_dss_device *dssdev);
		configure_overlay_size(dssdev);
#endif		
// == 2011.06.12 === hycho@ubiquix.com END
...
#if defined(CONFIG_PRODUCT_LGE_HUB) || defined(CONFIG_PRODUCT_LGE_JUSTIN)  // == 2011.05.30 === hycho@ubiquix.com
...
#if defined(CONFIG_PRODUCT_LGE_HUB) || defined(CONFIG_PRODUCT_LGE_JUSTIN) 	// == 2011.06.27 === jslee@ubiquix.com	hub froyo code merge
...
void configure_overlay_size(struct omap_dss_device *dssdev)
{
	...
	printk(KERN_INFO "[hycho0] configure_overlay_size\n");
	c = &dss_cache.overlay_cache[plane];
...
#define _720X480P	2 // TMDL_HDMITX_VFMT_02_720x480p_60Hz
	...
	printk(KERN_INFO "[hycho1] configure_overlay_size\n");		
}

#if defined(CONFIG_PRODUCT_LGE_HUB)
void configure_overlay_size_rollack(void)
{
	struct overlay_cache_data *c;
	struct omap_overlay *ovl;
	int is_lcd = 1;
	...
	printk(KERN_INFO "[IN] configure_overlay_size_rollack = %x\n", c->paddr);
...
#define _720X480P	2 // TMDL_HDMITX_VFMT_02_720x480p_60Hz
	...
	configure_overlay(plane);
	printk(KERN_INFO "[OUT] configure_overlay_size_rollack\n");		
	HDMItoLCD_Configure_pass = 0;
}
#endif
#endif
