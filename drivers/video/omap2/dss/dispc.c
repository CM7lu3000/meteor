...
#if defined(CONFIG_MACH_LGE_JUSTIN)
// LGE_JUSTIN_S 20110317 kyungrae.jo@lge.com, LUT#14
...
0x00FFF9FF, 
0x00FFFAFF};
// LGE_JUSTIN_E 20110317kyungrae.jo@lge.com, LUT#14
#elif CONFIG_GAMMA_TUNING
...
{
	printk("dispc_set_ccs_matrix\n");
#define CVAL(x, y) (FLD_VAL(x, 26, 16) | FLD_VAL(y, 10, 0))

	dispc_write_reg(DISPC_VID_CONV_COEF(0, 0), CVAL(ccs_info->rcr, ccs_info->ry));
	dispc_write_reg(DISPC_VID_CONV_COEF(0, 1), CVAL(ccs_info->gy,	 ccs_info->rcb));
	dispc_write_reg(DISPC_VID_CONV_COEF(0, 2), CVAL(ccs_info->gcb, ccs_info->gcr));
	dispc_write_reg(DISPC_VID_CONV_COEF(0, 3), CVAL(ccs_info->bcr, ccs_info->by));
	dispc_write_reg(DISPC_VID_CONV_COEF(0, 4), CVAL(0,       ccs_info->bcb));

	dispc_write_reg(DISPC_VID_CONV_COEF(1, 0), CVAL(ccs_info->rcr, ccs_info->ry));
	dispc_write_reg(DISPC_VID_CONV_COEF(1, 1), CVAL(ccs_info->gy,	 ccs_info->rcb));
	dispc_write_reg(DISPC_VID_CONV_COEF(1, 2), CVAL(ccs_info->gcb, ccs_info->gcr));
	dispc_write_reg(DISPC_VID_CONV_COEF(1, 3), CVAL(ccs_info->bcr, ccs_info->by));
	dispc_write_reg(DISPC_VID_CONV_COEF(1, 4), CVAL(0,       ccs_info->bcb));
				...
				u16 *x_decim, u16 *y_decim, bool *three_tap)
{
	...
	DSSDBG("dispc_scaling_decision (%d, %d), (%d, %d), %d "
		"cmode %x, chan %d rot %d "
		"%d, %d, %d, %d\n",
		   width, height, out_width, out_height, plane,
		   color_mode, channel, rotation,
		   min_x_decim, max_x_decim, min_y_decim, max_y_decim);
...
int HDMItoLCD_Interrupts_pass = 0;
int HDMItoLCD_Configure_pass = 0;

void HDMItoLCD_reg_rollback(void)
{
#define DUMPREG(r) printk(KERN_INFO "[dispc_dump] %08x\n", dispc_read_reg(r))

	dss_clk_enable(DSS_CLK_ICK | DSS_CLK_FCK1);

	dispc_write_reg(DISPC_TIMING_H, 0x0);	
	dispc_write_reg(DISPC_TIMING_V, 0x0);	
	dispc_write_reg(DISPC_POL_FREQ, 0x0);
	dispc_write_reg(DISPC_DIVISOR, 0x10003);
	dispc_write_reg(DISPC_CONTROL, 0x18b08);
...
static void _omap_dispc_initial_config(void)
{
	...
	/* FUNCGATED */
//--[[ LGE_UBIQUIX_MODIFIED_START : hycho@ubiquix.com [2011.07.15] - Added Hub Lcd color problem	
...
#if defined(CONFIG_GAMMA_TUNING) || defined(CONFIG_PRODUCT_LGE_JUSTIN)
...
void dispc_set_loadmode_resume(enum omap_dss_load_mode mode)
{
	u32 l;	
...
//	_dispc_setup_color_conv_coef();
	...
	REG_FLD_MOD(DISPC_CONFIG, 1, 18, 18); // Alpha blending
}
