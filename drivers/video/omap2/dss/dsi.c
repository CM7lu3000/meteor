...
#if defined(CONFIG_PRODUCT_LGE_HUB) || defined(CONFIG_PRODUCT_LGE_JUSTIN) || defined(CONFIG_PRODUCT_LGE_BLACK) // 20110627 jslee@ubiqui.com
#else
...
//--[[ LGE_UBIQUIX_MODIFIED_START : jslee@ubiquix.com [2011.06.27] - justin froyo source merge. 
static void dsi_perf_mark_start_auto(enum omap_dsi_index lcd_ix)
{
	struct dsi_struct *p_dsi;
...
//--[[ LGE_UBIQUIX_MODIFIED_END : jslee@ubiquix.com [2011.06.27] - justin froyo source merge. 

static void dsi_perf_show(enum omap_dsi_index ix, const char *name)
{
...
#define dsi_perf_mark_start_auto(lcd_ix)     // 20110627 jslee@ubiquix.com
...
//	printk("[hycho] dsi_irq_handler =%x\n", irqstatus);
#ifdef CONFIG_OMAP2_DSS_COLLECT_IRQ_STATS
...
//--[[ LGE_UBIQUIX_MODIFIED_START : jslee@ubiquix.com [2011.06.27] - j_froyo merge
	...
	-1 & (~(DSI_CIO_IRQ_ERRCONTROL2|DSI_CIO_IRQ_ERRCONTENTIONLP0_1|DSI_CIO_IRQ_ERRCONTENTIONLP1_1)));
	printk("[jslee] dsi.c - dsi_irqstatus_7(DSI_COMPLEXIO_IRQ_ENABLE : 0x%08X)\n"
		, dsi_read_reg(ix, DSI_COMPLEXIO_IRQ_ENABLE));
#else
...
{
	REG_FLD_MOD(0, DSI_CTRL, enable, 0, 0); /* IF_EN */
}
EXPORT_SYMBOL(dsi_if_enable_hdmi);
/* TI PATCH */
#endif
...
/* 20100623 kyungtae.oh@lge.com hub didn't use vdds [LGE_START]*/
//--[[ LGE_UBIQUIX_MODIFIED_START : scchoi@ubiquix.com [2011.07.04] - Added black feature. 
#if defined(CONFIG_MACH_LGE_HUB) || defined(CONFIG_PRODUCT_LGE_JUSTIN) || defined(CONFIG_PRODUCT_LGE_BLACK)  // 20110627 jslee@ubiquix.com
#else
...
int dsi_vc_send_bta_sync_for_justin(enum omap_dsi_index lcd_ix, int channel)
{
	int r = 0;
	u32 err;
	struct dsi_struct *p_dsi;
...
int dsi_vc_set_max_rx_packet_size_for_justin(enum omap_dsi_index lcd_ix, int channel, u16 len)
{
	...
	r = dsi_vc_send_bta_sync_for_justin(lcd_ix, channel);
#endif
	return r;
}
EXPORT_SYMBOL(dsi_vc_set_max_rx_packet_size_for_justin);
//jkr test end, Send BTA
#endif
//--[[ LGE_UBIQUIX_MODIFIED_END : jslee@ubiquix.com [2011.06.27] - j_froyo merge
...
static int dsi_proto_config(struct omap_dss_device *dssdev)
{
	...
	r = FLD_MOD(r, (cpu_is_omap44xx()) ? 1 : 0,
			9, 9);	/*VP_DE_POL */
#if defined(CONFIG_PRODUCT_LGE_JUSTIN)//natting_test: TE active High
...
//--[[ LGE_UBIQUIX_MODIFIED_START : jslee@ubiquix.com [2011.06.27] - j_froyo merge
#if defined(CONFIG_PRODUCT_LGE_JUSTIN)
// 20110627 jslee@ubiquix.com start. (froyo source merge)
void dsi_DDR_CLK_ALWAYS_ON(enum omap_dsi_index lcd_ix, int enable)
{
  u32 regv;
  u32 l;
  
  regv=dsi_read_reg(lcd_ix, DSI_REG(0x0054));

  l=FLD_MOD(regv, enable, 13, 13);

  dsi_write_reg(lcd_ix, DSI_REG(0x0054), l);
}
// 20110627 jslee@ubiquix.com end. (froyo source merge)
#endif
//--[[ LGE_UBIQUIX_MODIFIED_END : jslee@ubiquix.com [2011.06.27] - j_froyo merge
...
//--[[ LGE_UBIQUIX_MODIFIED_START : jslee@ubiquix.com [2011.06.27] - j_froyo merge
#if defined(CONFIG_PRODUCT_LGE_JUSTIN) //temp, BTA rollback
	REG_FLD_MOD(ix, DSI_VC_CTRL(0), 1, 6, 6); /* BTA_EN */ //jkr test, Send BTA

	dsi_DDR_CLK_ALWAYS_ON(ix, 0);	//jkr test, Send BTA
	__raw_writel(0x05, dsi1.base + 0x0110); //jkr test, Send any command to trigger CLK Off
//--[[ LGE_UBIQUIX_MODIFIED_END : jslee@ubiquix.com [2011.06.27] - j_froyo merge	
#else
	/* RX_FIFO_NOT_EMPTY */
	if (REG_GET(ix, DSI_VC_CTRL(channel), 20, 20)) {
...
#if defined(CONFIG_PRODUCT_LGE_HUB) || defined(CONFIG_PRODUCT_LGE_JUSTIN)
	if(HDMItoLCD_Interrupts_pass) // == 2011.06.18 === hycho@ubiquix.com    
	{
		printk(KERN_INFO "================ dsi_framedone_irq_callback\n");		
		return;
	}
#endif
...
//--[[ LGE_UBIQUIX_MODIFIED_START : jslee@ubiquix.com [2011.06.27] - j_froyo merge
void dsi_phy_config_4_bta(enum omap_dsi_index ix)
{
  //u32 regv;
  
  //regv=dsi_read_reg(DSI_DSIPHY_CFG1);
  REG_FLD_MOD(ix, DSI_DSIPHY_CFG1, 0x00, 31, 24);
}
//--[[ LGE_UBIQUIX_MODIFIED_END : jslee@ubiquix.com [2011.06.27] - j_froyo merge
...
{
	//return -1: No Ack (No LCD)
	//return 1 : Ack (LCD exist)
	...
	dsi_DDR_CLK_ALWAYS_ON(lcd_ix, 1);
	//dsi_phy_config_4_bta(lcd_ix);
	dsi_vc_set_max_rx_packet_size_for_justin(lcd_ix, dssdev->channel, 1);
	// BTA ONLY
	//bta_result = dsi_vc_send_bta_sync_for_justin(lcd_ix, dssdev->channel);
	REG_FLD_MOD(lcd_ix, DSI_VC_CTRL(0), 1, 6, 6); /* BTA_EN */
	// DSI_IRQSTATUS Check
	//bta_result = dsi_BTA_IRQ_check(lcd_ix);
	//printf("BTA TEST: Ack result = %d\n", bta_result);
	dsi_DDR_CLK_ALWAYS_ON(lcd_ix, 0);
	return bta_result;
}
#endif
//--[[ LGE_UBIQUIX_MODIFIED_END : jslee@ubiquix.com [2011.06.27] - j_froyo merge

//--[[ LGE_UBIQUIX_MODIFIED_START : ymjun@mnbt.co.kr [2011.08.07] - CAM color temp.
...
// LGE_DOM_UPDATE_S hyewon.eum 2010/10/01 }
//--]] LGE_UBIQUIX_MODIFIED_END : ymjun@mnbt.co.kr [2011.08.07] - CAM color temp.
...
/* 20100623 kyungtae.oh@lge.com hub didn't use vdds [LGE_START]*/
//--[[ LGE_UBIQUIX_MODIFIED_START : scchoi@ubiquix.com [2011.07.04] - Added black feature. 
#if defined(CONFIG_PRODUCT_LGE_HUB) || defined(CONFIG_PRODUCT_LGE_JUSTIN) || defined(CONFIG_PRODUCT_LGE_BLACK)   // 20110627 jslee@ubiquix.com
#else
...
void dsi1_queue_flush(void)
{
	flush_workqueue(dsi1.update_queue);	
	flush_workqueue(dsi1.workqueue);
}
#endif
