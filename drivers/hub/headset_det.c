...
#if defined(CONFIG_PRODUCT_LGE_BLACK)||defined(CONFIG_PRODUCT_LGE_JUSTIN)||defined(CONFIG_PRODUCT_LGE_HUB) // 20110105 seungdae.goh@lge.com justin gpio adjust // 20110527 bsnoh@ubiquix.com add Hub Product
#define USE_LDOBIAS 1 //B-PRJ, Justin : use mic bias by gpio_en
#endif

#ifdef USE_LDOBIAS
#include "../mux.h"
#if defined(CONFIG_PRODUCT_LGE_JUSTIN)||defined(CONFIG_PRODUCT_LGE_HUB)// 20101214 seungdae.goh@lge.com justin gpio adjust
...
#if defined(CONFIG_PRODUCT_LGE_BLACK)
	//nothing
#else
...
static void set_wakelock(u32 set) {
	...
	if (set > 0) {
		if(!headset_wlock.wake_lock_on)
		{
			...
			printk(KERN_WARNING "[Headset_Det] set_wakelock = 1 \n");
		}
	} else {
		if(headset_wlock.wake_lock_on)
		{
			wake_unlock(&headset_wlock.wake_lock);
			printk(KERN_WARNING "[Headset_Det] set_wakelock = 0 \n");
		}
	}
	headset_wlock.wake_lock_on = set;

	printk(KERN_WARNING "[Headset_Det] wake_lock : %s\n", (set ? "on" : "off"));
}
// 20110425 mikyoung.chang@lge.com detect headset during sleep [END]
...
#if defined(CONFIG_PRODUCT_LGE_BLACK) || defined(CONFIG_PRODUCT_LGE_JUSTIN)	// 20100814 junyeop.kim@lge.com, dmb ant detect [START_LGE]
	//nothing
#elif defined(CONFIG_PRODUCT_LGE_HUB)
...
//		schedule_delayed_work(&headset_sw_data->delayed_work,	msecs_to_jiffies(type_detection_tim*3));
	...
	if(hook_status == HOOK_RELEASED){
		if(gpio_get_value(headset_sw_data->hook_gpio) == 0){ 	//threshold area for hook key glitch
		    ...
		    hook_status = HOOK_PRESSED; 
//		    input_report_key(headset_sw_data->ip_dev, KEY_HOOK, 1);
#if defined(CONFIG_PRODUCT_LGE_JUSTIN) || defined(CONFIG_PRODUCT_LGE_BLACK) //20110404 mikyoung.chang@lge.com B HW Request.roll back the delay time.
...
static irqreturn_t headset_int_handler(int irq, void *dev_id)
{
	...
	printk("[JIWON] headset_int_handler: IsHeadsetInserted = 0 \n");
	// 20110425 mikyoung.chang@lge.com detect headset during sleep [START]	
	set_wakelock(1);
	// 20110425 mikyoung.chang@lge.com detect headset during sleep [END]
...
static irqreturn_t headset_hook_int_handler(int irq, void *dev_id)
{
	...
	/* 20110215 jiwon.seo@lge.com for EarJack & Hook Action [START] */
			if(IsHeadsetInserted ==0) 
			{
				...
				printk("[JIWON] headset_hook_int_handler: IsHeadsetInserted \n");
	/* 20110215 jiwon.seo@lge.com for EarJack & Hook Action [END] */
...
#if defined(CONFIG_PRODUCT_LGE_BLACK) || defined(CONFIG_PRODUCT_LGE_HUB)// 20100814 junyeop.kim@lge.com, dmb ant detect [START_LGE]
	//nothing
#else
