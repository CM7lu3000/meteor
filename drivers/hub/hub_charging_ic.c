...
static int charging_ic_probe(struct platform_device *dev)
{
	...
	gpio_set_debounce(CHG_STATUS_N_OMAP, 0xa);
...
{
#if !defined(CONFIG_PRODUCT_LGE_HUB) /* 20110720, mschung@ubiquix.com, Block. HUB should not boot-up after SW-PowerOff even if TA connected. */
/* LGE_CHANGE_S [daewung.kim@lge.com] 2010-12-27, Reboot rather than pwr off when TA inserted */
