...
#define DMA_MIN_BYTES			8		/* scchoi - merge justin_froyo */
...
#endif
// LGE_UPDATE_E eungbo.shim@lge.com [EBS]
// LGE_DOM_UPDATE_S hayun.kim 2010/06/30 { /* CONFIG_LGE_BROADCAST_SPI */
...
#if 0 //- [2011. 05. 20] shyun modify - duplicate function.
...
static void omap2_mcspi_restore_ctx(struct omap2_mcspi *mcspi)
{
	...
	list_for_each_entry(cs, &omap2_mcspi_ctx[spi_cntrl->bus_num - 1].cs, node)
		__raw_writel(cs->chconf0, cs->base +	mcspi->regs[OMAP2_MCSPI_CHCONF0]);
}
...
// LGE_UPDATE_S eungbo.shim@lge.com [EBS] For DMA FIFO 
static int omap2_mcspi_dump_regs(struct spi_device *spi)                                                  
{                                                                                              
 struct spi_master *master = spi->master;                                                      
...
// LGE_UPDATE_S eungbo.shim@lge.com [EBS] For BURST Mode Setting 
	...
	{
		chk_set = 1;
		omap_dma_set_global_params(DMA_DEFAULT_ARB_RATE, 0x20, 1); // SPI_IPC	/* SCCHOI - check */
...
// LGE_UPDATE_E eungbo.shim@lge.com [EBS]
	...
	if (tx != NULL) 
	{
            //20110329 ws.yang@lge.com .. to Through-put requirement 
...
// LGE_UPDATE_S eungbo.shim@lge.com [EBS] SPI Retry Routine 
		...
		if (mcspi->mcspi_mode == OMAP2_MCSPI_SLAVE) //For RIL SPI 
		{
			...
			do 
			{
				wait_ret = wait_for_completion_timeout(&mcspi_dma->dma_tx_completion,1*HZ) ;  /* 20110304 dongyu.gwak@lge.com Retry Scheme */
				if( wait_ret == 0 )
				{
					...
					if(tmp_dma_read  == 0)
					{
						...
						printk(KERN_ERR "<< SPI v.S14 Timeout occurred TX, Transfer not started\n" );
					}
					else
					{
						// transfer started.
						...
						printk(KERN_ERR "<< SPI v.S14 Timeout occurred TX, Transfer started CCEN %lu CCFN %lu\n", tmp_dma_read, tmp_dma_read1 );
						omap2_mcspi_dump_regs(spi);
					}
					//return -ETIMEDOUT;
//#ifdef CONFIG_SPI_DEBUG
...
//#endif
	...
	if (rx != NULL) 
	{
	//20110329 ws.yang@lge.com .. add to Through-put requirement 
		omap_pm_set_min_bus_tput(&spi->dev,OCP_INITIATOR_AGENT, 800000);	//omap-pm-srf.c
...
// LGE_UPDATE_S eungbo.shim@lge.com [EBS] -- For SPI Retry Routine 
		...
		if (mcspi->mcspi_mode == OMAP2_MCSPI_SLAVE)
		{
			...
			do 
			{
				...
				wait_ret_rx = wait_for_completion_timeout(&mcspi_dma->dma_rx_completion, 1*HZ); // 1tick == 8ms, 3000 * 8ms = 24,000ms
				if( wait_ret_rx == 0 )
				{
					...
					{
						// transfer didn't start
						printk(KERN_ERR "<< SPI v.S14 Timeout occurred RX, Receiving not started\n" );
					}
					else
					{
						// transfer started.
						...
						printk(KERN_ERR "<< SPI v.S14 Timeout occurred RX, Receiving started CCEN %lu CCFN %lu\n", tmp_dma_read_rx, tmp_dma_read1_rx );
						omap2_mcspi_dump_regs(spi);
					}
					//return -ETIMEDOUT;
//#ifdef CONFIG_SPI_DEBUG
...
// LGE_UPDATES_E eungbo.shim@lge.com [EBS] Move to slave Rdy Code  
	     ...
	     omap_pm_set_min_bus_tput(&spi->dev, OCP_INITIATOR_AGENT, 0); 		
	...
	if (word_len <= 8) 
	{
		...
		do 
		{
			...
			if (rx != NULL) 
			{
				...
				if (c == 1 && tx == NULL && (l & OMAP2_MCSPI_CHCONF_TURBO)) 
				{
					omap2_mcspi_set_enable(spi, 0);
...
static int omap2_mcspi_setup_transfer(struct spi_device *spi, struct spi_transfer *t)
{
	...
	if (speed_hz) 
	{
		while (div <= 15 && (OMAP2_MCSPI_MAX_FREQ / (1 << div)) > speed_hz)
			div++;
	} 
	else
	{
	...
	while (!list_empty(&mcspi->msg_queue)) 
	{
		...
		list_for_each_entry(t, &m->transfers, transfer_list) 
		{
			...
			if (t->len) 
			{
				...
				if (t->tx_buf == NULL)
					__raw_writel(0, cs->base + mcspi->regs[OMAP2_MCSPI_TX0]);

				if (m->is_dma_mapped 
#ifdef CONFIG_SPI_TDMB // LGE_DOM_UPDATE_S hayun.kim 2010/06/30 CONFIG_LGE_BROADCAST_SPI
...
// LGE_UPDATE_S eungbo.shim@lge.com [EBS] for make multiple SPI thread 	
	...
	mcspi->wq = create_rt_workqueue(mcspi->name);//create_singlethread_workqueue
#endif
// LGE_UPDATE_E eungbo.shim@lge.com [EBS] for make multiple SPI thread	
	...
	for (i = 0; i < pdata->num_cs; i++) 
	{
		...
		dma_res = platform_get_resource_byname(pdev, IORESOURCE_DMA, dma_ch_name);
		if (!dma_res) {
