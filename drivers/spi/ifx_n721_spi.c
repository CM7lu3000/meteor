...
#if (defined(CONFIG_DOMESTIC) && defined(CONFIG_JUSTIN_SPI)) // LGE_JUSTIN_S 20101209 mikyoung.chang@lge.com, justin one git 
#define MODEM_CHK 65 //gpio_65 AP_SLEEP_CHK [LGE-SPI]
#else
#define MODEM_CHK 177 //gpio_177 AP_SLEEP_CHK [LGE-SPI]
#endif // LGE_JUSTIN_E 20101209 mikyoung.chang@lge.com, justin one git
/* ################################################################################################################ */
...
#else
	wait_for_completion_timeout(&spi_data->ifx_read_write_completion, 3*HZ);// 10hz -> 3hz change to eungbo.shim@lge.com  	
#endif
	if(spi_data->ifx_ret_count==0)
	{	
//		lge_debug[D_SPI].enable = 1;
		ifx_spi_set_mrdy_signal(spi_data, 0);
		printk("%s:%d - timeout!! Can't get SRDY from CP for 3Sec. Set MRDY high to low \n", __FUNCTION__, __LINE__);
//		dump_spi_buffer("ifx_spi_write -- fail()", buf, count);
	}
...
// LGE_UPDATE_S eungbo.shim@lge.com -- HUB Sync To block big siaze packet to upper layer
		if(rx_valid_buf_size > 2048)
		{
		 	printk(KERN_ERR "[LGE-SPI] rx_valid_size is over --->debug size =%d\n", rx_valid_buf_size);
			return ;
		}
// LGE_UPDATE_E eungbo.shim@lge.com -- HUB Sync To block big siaze packet to upper layer
