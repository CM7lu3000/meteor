...
#include <plat/omap-pm.h>//nthyunjin.yang for test
...
extern int usb_swiched_thru_factory_cmd;
...
static void early_suspend(struct work_struct *work)
{
	...
	struct device l3_dev;//nthyunjin.yang for test
    ...
    /* 20110331 sookyoung.kim@lge.com LG-DVFS [START_LGE] */
	omap_pm_set_min_bus_tput(&l3_dev, OCP_INITIATOR_AGENT, 100 * 1000 * 4); //nthyunjin.yang
	/*//nthyunjin.yang for test
...
static void late_resume(struct work_struct *work)
{
	...
	omap_pm_set_min_bus_tput(&l3_dev, OCP_INITIATOR_AGENT, 200 * 1000 * 4); //nthyunjin.yang
...
#if defined(CONFIG_PRODUCT_LGE_JUSTIN) 	/* 20110705, mschung@ubiquix.com, One Source, JUSTIN */
// START sungchae.koo@lge.com 2011/04/03 LAB1_FW : BLOCK_ENTERING_EARLYSUSPEND_STATE for factory test {
	printk("[request_suspend_state] factory=%d, old_state=%d, new_state=%d\n", usb_swiched_thru_factory_cmd, requested_suspend_state, new_state);

	if( 1==usb_swiched_thru_factory_cmd && PM_SUSPEND_MEM==new_state )
	{
		printk("[request_suspend_state] Entering early suspend state is blocked!\n");
		return;
	}
// END sungchae.koo@lge.com 2011/04/03 LAB1_FW }
#endif
