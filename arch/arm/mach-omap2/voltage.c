...
static void __init vdd_data_configure(struct omap_vdd_info *vdd)
{
	...
	(void) debugfs_create_file("vp_errorgain", S_IRUGO | S_IWUSR | S_IWGRP,
				vdd_debug,
				&(vdd->vp_reg.vpconfig_errorgain),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_smpswaittimemin", S_IRUGO | S_IWUSR | S_IWGRP,
				vdd_debug,
				&(vdd->vp_reg.vstepmin_smpswaittimemin),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_stepmin", S_IRUGO | S_IWUSR | S_IWGRP, vdd_debug,
				&(vdd->vp_reg.vstepmin_stepmin),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_smpswaittimemax", S_IRUGO | S_IWUSR | S_IWGRP,
				vdd_debug,
				&(vdd->vp_reg.vstepmax_smpswaittimemax),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_stepmax", S_IRUGO | S_IWUSR | S_IWGRP, vdd_debug,
				&(vdd->vp_reg.vstepmax_stepmax),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_vddmax", S_IRUGO | S_IWUSR | S_IWGRP, vdd_debug,
				&(vdd->vp_reg.vlimitto_vddmax),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_vddmin", S_IRUGO | S_IWUSR | S_IWGRP, vdd_debug,
				&(vdd->vp_reg.vlimitto_vddmin),
				&vp_debug_fops);
	(void) debugfs_create_file("vp_timeout", S_IRUGO | S_IWUSR | S_IWGRP, vdd_debug,
				&(vdd->vp_reg.vlimitto_timeout),
				&vp_debug_fops);
	...
	if (cpu_is_omap44xx() && !strcmp("vdd_iva", name))
		(void) debugfs_create_u8("fbb_enable", S_IRUGO | S_IWUSR | S_IWGRP,
