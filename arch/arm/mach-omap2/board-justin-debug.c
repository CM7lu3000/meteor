/*
 * linux/arch/arm/mach-omap2/board-justin-debug.c
 *
...
struct platform_device justin_ram_console_device = {
	.name = "ram_console",
	.id = -1,
	.num_resources = 1,
...
static int lge_justin_panic_event(struct notifier_block *this,
...
static int lge_justin_reboot_event(struct notifier_block *this,
...
struct notifier_block lge_justin_panic_nb = {
	.notifier_call = lge_justin_panic_event,
};

struct notifier_block lge_justin_reboot_nb = {
	.notifier_call = lge_justin_reboot_event,
};

extern int register_reboot_notifier(struct notifier_block *nb);

static int justin_debug_init(void)
{
printk("%s\n", __func__);
	atomic_notifier_chain_register(&panic_notifier_list, &lge_justin_panic_nb);
	register_reboot_notifier(&lge_justin_reboot_nb);
	return 0;
}

module_init(justin_debug_init);
